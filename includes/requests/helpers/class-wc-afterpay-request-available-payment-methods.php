...
class WC_AfterPay_Request_Available_Payment_Methods extends WC_AfterPay_Request {
	...
	private $request_path   = '/api/v3/checkout/payment-methods';
	 ...
	 */
	public function response( $personal_number, $customer_category ) {
		$request_url = 'https://sandboxapi.horizonafs.com/eCommerceServicesWebApi' . $this->request_path;
		$request     = wp_remote_retrieve_body( wp_remote_request( $request_url, $this->get_request_args( $personal_number, $customer_category ) ) );
		return $request;
	}
	/**
	 ...
	 * @return array
	 */
	private function get_request_args( $personal_number, $customer_category ) {
		$request_args = array(
			'headers' => $this->request_header(),
			'body'    => $this->get_request_body( $personal_number, $customer_category ),
	 ...
	 * @return false|string
	 */
	private function get_request_body( $personal_number, $customer_category ) {
		$formatted_request_body = array(
			'customer'       	=> array('customerCategory' => $customer_category, 'identificationNumber' => $personal_number ),
